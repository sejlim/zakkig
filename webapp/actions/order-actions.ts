...
export async function placeOrderAction(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Bestellung konnte nicht aufgegeben werden.'
...
export async function updateOrderStatusAction(orderId: string, status: string, organizationId: string) {
  try {
...
export async function createKitchenSessionAction(organizationId: string) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Session konnte nicht erstellt werden.'
    return { error: message }
  }
}

export async function deleteKitchenSessionAction(sessionId: string, organizationId: string) {
  const user = await getUser()
  if (!user) return { error: 'Nicht authentifiziert.' }
  try {
    ...
    const message = error instanceof Error ? error.message : 'Session konnte nicht gelöscht werden.'
    return { error: message }
  }
}
