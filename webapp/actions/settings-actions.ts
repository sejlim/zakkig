...
export async function updateBusinessAction(
  ...
  try {
    ...
    if (logoFile && logoFile.size > 0) {
      if (existingLogoId) {
        try { await deleteMenuImage(existingLogoId) } catch { /* ignore */ }
      }
    ...
    const message = error instanceof Error ? error.message : 'Daten konnten nicht gespeichert werden.'
...
export async function toggleFeatureAction(
  ...
  try {
    const updateData = feature === 'to-go' 
      ? { isToGoEnabled: value } 
    ...
    const message = error instanceof Error ? error.message : 'Feature konnte nicht umgeschaltet werden.'
    return { error: message }
  }
}
