import { getOrganization, getMenuCategories, getAvailableMenuItems } from '@/lib/appwrite/database'
...
export default async function ToStayPage({
  ...
  const [{ organizationId }, { table }] = await Promise.all([params, searchParams])
  ...
  if (!organization) return <div className="p-8 text-center">Organization not found</div>
    ...
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center p-6 bg-background text-center">
        <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mb-6">
          <Storefront className="w-8 h-8 text-muted-foreground" weight="duotone" />
        </div>
        <h1 className="text-2xl font-bold mb-2">Bestellungen pausiert</h1>
        <p className="text-muted-foreground max-w-sm">
          {organization.name} nimmt momentan keine Tisch-Bestellungen über das Handy an. Bitte wende dich an unser Personal.
        </p>
      </div>
    )
  }
