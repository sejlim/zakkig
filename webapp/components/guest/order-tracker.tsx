...
export function OrderTracker({ orderId, organization, initialOrder }: OrderTrackerProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-muted/10">
      <header className="bg-background border-b px-6 py-4 flex items-center justify-between">
        <div className="w-10" />
        <h1 className="text-lg font-bold flex-1 text-center">{organization.name}</h1>
      ...
      <main className="flex-1 p-4 md:p-6 max-w-md mx-auto w-full flex flex-col items-center justify-center space-y-8">
        <div className="text-center">
          ...
          <div className="text-4xl font-mono font-bold">{order.orderNumber}</div>
        </div>
              ...
              {statusSteps.map((step, index) => {
                ...
                return (
                  <div key={step.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-sm z-10 ${isCompleted ? 'border-primary text-primary' : 'border-muted text-muted-foreground bg-background'} ${isCurrent ? '!bg-primary text-primary-foreground border-primary' : ''}`}>
                      {isCompleted && !isCurrent ? (
                        <svg className="w-5 h-5 fill-current" viewBox="0 0 20 20"><path d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" fillRule="evenodd"></path></svg>
                      ) : (
                        <span className="text-sm font-bold">{index + 1}</span>
                      )}
                    </div>
                    <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl shadow-sm border ${isCurrent ? 'border-primary/50 bg-primary/5' : 'bg-background'}`}>
                      <h3 className={`font-bold ${isCurrent ? 'text-primary' : (isCompleted ? '' : 'text-muted-foreground')}`}>
        ...
        {order.status === 'completed' && (
          <Button variant="outline" className="w-full" onClick={() => window.location.href = `/to-go/${organization.$id}`}>
            Neue Bestellung
          </Button>
        )}
      </main>
