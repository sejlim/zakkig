...
export function KitchenBoard({ organization, initialOrders }: KitchenBoardProps) {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setOrders(prev => prev.filter(order => {
        if (order.status !== 'completed' && order.status !== 'ready') return true;
  ...
  useEffect(() => {
    const unsubscribe = subscribeToOrders(organization.$id, (response) => {
      if (response.events.includes('databases.*.collections.*.documents.*.create')) {
        ...
        setOrders((prev) => 
          prev.map((o) => (o.$id === response.payload.$id ? (response.payload as unknown as Order) : o))
        )
  ...
  async function handleMarkDone(order: Order) {
    ...
    setOrders(prev => prev.map(o => o.$id === order.$id ? { ...o, status: newStatus, $updatedAt: new Date().toISOString() } : o))
  ...
  function formatAge(createdAt: string) {
    const diffMins = Math.floor((currentTime - new Date(createdAt).getTime()) / 60000)
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-muted/20 overflow-hidden">
      <header className="flex h-14 shrink-0 items-center justify-between border-b bg-background px-6 shadow-sm">
        <h1 className="text-xl font-bold tracking-tight">zakkig <span className="font-normal text-muted-foreground ml-2">Kitchen Board</span></h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeOrders.map(order => (
                <OrderCard 
                  ...
                  actionButton={
                    <Button className="w-full mt-4" size="lg" onClick={() => handleMarkDone(order)}>
            ...
            <div className="flex flex-col gap-4">
              {completedOrders.map(order => (
                <OrderCard 
                  ...
                  age={formatAge(order.$createdAt)}
                  actionButton={
                    <Button variant="outline" className="w-full mt-4" onClick={() => handleClearOrder(order)}>
...
function OrderCard({ order, items, age, actionButton }: { order: Order, items: OrderItem[], age: string, actionButton: React.ReactNode }) {
  const { t } = useTranslation()
